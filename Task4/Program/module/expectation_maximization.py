...
n_iters_range = [40, 60, 80, int(1e2), 200, int(1e3), int(1e4), int(1e5), int(1e6), int(1e7),
                 int(1e8), int(1e9), int(1e10)]
...
def expectation_maximization_clustering(data_set: np.ndarray, data_set_name: str,
    ...
    for n_clusters in n_clusters_range:
        ...
        if save_latex:
            file_name = data_set_name + "_" + str(n_clusters) + "_" \
                        + datetime.now().strftime("%H%M%S")
            latex_generator.generate_vertical_table(
                header_names=["Max iterations", "full", "tied", "diag", "spherical"],
