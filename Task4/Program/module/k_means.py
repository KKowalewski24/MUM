...
# te wartosci zostaly wybrane "na oko", bazujac na wykresach
# generowanych przy badaniu Cluster-number
CLUSTERS_NUMBER_TO_MAX_ITER = [2, 6, 8, 19, 25]
MAX_ITER = [int(float("1e" + str(x))) for x in range(2, 12)]
    ...
    for cluster_value in CLUSTERS_NUMBER_TO_MAX_ITER:
        for iter_value in MAX_ITER:
            ...
            if not is_all_items_same:
                ...
                print("Clusters number: " + str(cluster_value) + "\tMax iterations number:\t"
                      + str(iter_value) + "\tSilhouette:\t" + str(score))

    if save_latex:
        _draw_and_save_chart(scores_clusters_numbers, scores_iter_numbers, data_set_name)
        _save_score(scores_clusters_numbers, scores_iter_numbers, data_set_name)


def _save_score(score_clusters: List[List[float]],
                score_iters: List[List[float]], data_set_name: str) -> None:
    ...
    for cluster_number in CLUSTERS_NUMBER_TO_MAX_ITER:
        latex_generator.generate_vertical_table(
            ...
            filename="kmeans_table_iters" + filename_description + "_"
                     + str(cluster_number) + "_clusters"
        )
        index += 10


def _draw_and_save_chart(score_clusters: List[List[float]],
                         score_iters: List[List[float]], data_set_name: str) -> None:
    ...
    index = 0
    for cluster_number in CLUSTERS_NUMBER_TO_MAX_ITER:
        plt.plot(*zip(*score_iters[index:index + 10]), colors[cluster_number],
                 label=str("Clusters: " + str(cluster_number)))
        index += 10

    plt.ylabel("Silhouette")
    plt.xlabel("Max Iterations number")
    plt.legend(bbox_to_anchor=(0., 1.02, 1., .102), loc='upper left',
               fontsize='xx-small', ncol=5, mode="expand")
